...
@app.route('/hapus-laporan-masuk/<string:no_plat>')
def deleteLapoarnMasuk(no_plat):
    cur = mysql.connection.cursor()
    cur.execute('DELETE FROM laporan_masuk WHERE no_plat = %s',(no_plat,))
    mysql.connection.commit()
    return redirect(url_for('tampillaporanmasuk')) 

@app.route('/hapus-laporan-keluar/<string:no_plat>')
def deleteLapoarnKeluar(no_plat):
    cur = mysql.connection.cursor()
    cur.execute('DELETE FROM laporan_keluar WHERE no_plat = %s',(no_plat,))
    mysql.connection.commit()
    return redirect(url_for('tampillaporankeluar')) 

@app.route('/hapus-karyawan/<int:id_krywn>')
def deleteKaryawan(id_krywn):
    cursor = mysql.connection.cursor()
    cursor.execute('DELETE FROM karyawan WHERE id_krywn = %s', (id_krywn,))
    mysql.connection.commit()
    cursor.close()
    return redirect(url_for('data')) 

@app.route('/update-karyawan/<int:id_krywn>', methods=['GET', 'POST'])
def update_post(id_krywn):
    if request.method == 'POST':
        ...
        with mysql.connection.cursor() as cur:
            cur.execute('UPDATE karyawan SET nipy = %s, nama = %s, plat_nomor = %s, status = %s WHERE id_krywn = %s',
                        (nipy, nama, plat_nomor, status, id_krywn))
            mysql.connection.commit()
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM karyawan WHERE id_krywn = %s', (id_krywn,))
        rv = cursor.fetchone()
        return render_template("updateKaryawan.html", rv=rv)
